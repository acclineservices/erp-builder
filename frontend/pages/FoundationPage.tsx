...
const stateCopy: Record<BackendHealthState, { label: string; detail: string }> = {
  checking: { label: "Checking connection", detail: "Connecting to the Pruvian API." },
  available: { label: "Backend connected", detail: "The Pruvian API is available." },
    ...
    <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-950 dark:bg-slate-950 dark:text-slate-50 sm:px-10 lg:px-16">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-6xl flex-col">
        <header className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-xl bg-blue-700 font-semibold text-white shadow-sm">{applicationBrand.shortName}</div>
          <span className="text-lg font-semibold tracking-tight">{applicationBrand.productName}</span>
        </header>
        ...
        <section className="flex flex-1 items-center py-16 sm:py-24">
          <div className="grid w-full gap-10 lg:grid-cols-[1fr_22rem] lg:items-end">
            <div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.16em] text-blue-700 dark:text-blue-400">{applicationBrand.description}</p>
              <h1 className="max-w-3xl text-4xl font-semibold tracking-tight sm:text-6xl">{applicationBrand.tagline}</h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">
                {applicationBrand.productName} is being prepared to bring the confidence of familiar ERP workflows into a focused, modern SaaS experience.
              </p>
            </div>
