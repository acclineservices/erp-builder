...
def receipt(db: Session, company_id: UUID, document_id: UUID) -> GoodsReceipt | None: return db.scalar(select(GoodsReceipt).options(selectinload(GoodsReceipt.lines),selectinload(GoodsReceipt.supplier)).where(GoodsReceipt.id==document_id,GoodsReceipt.company_id==company_id))
def create_receipt(db: Session, company: Company, actor: User, payload: GoodsReceiptInput) -> GoodsReceipt:
    ...
    document=GoodsReceipt(company=company,supplier_id=payload.supplier_id,purchase_order_id=payload.purchase_order_id,warehouse_id=payload.warehouse_id,number=_next_number(db,company.id,"GRN"),receipt_date=payload.receipt_date,supplier_challan_reference=payload.supplier_challan_reference,notes=payload.notes,status="received")
    db.add(document); db.flush(); po_lines={line.id:line for line in linked.lines} if linked else {}
    ...
    if linked: linked.status="partially_received"
...
def cancel_receipt(db: Session, company: Company, actor: User, document: GoodsReceipt) -> GoodsReceipt:
    ...
    document.status="cancelled"; _audit(db,company,actor,"goods_receipt_cancelled",document); return document
